...
### Ability checks
When an action is risky and could fail — climbing, sneaking, persuading, hacking — the narrator asks for a check instead of deciding. The engine rolls d20 + stat (+ proficiency in your class's own stat) against the difficulty, shows the roll, and then tells the narrator the verdict to describe. Success pays XP; failure costs something.
